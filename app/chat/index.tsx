...
export default function ChatInboxScreen() {
  ...
  const { currentUser, tasks, messages } = useApp();
  const [query, setQuery] = useState("");
  const conversations = useMemo(
    () => buildConversations(tasks, messages, currentUser?.role),
    [currentUser?.role, messages, tasks]
  );
...
function buildConversations(tasks: Task[], messages: ChatMessage[], role?: string): Conversation[] {
  ...
  return visibleTasks.map((task, index) => {
    ...
    const participantName = role === "client" ? workerNames[index % workerNames.length] : "Maria Santos";
    ...
    return {
      ...
      unread: index === 0 && taskMessages.length > 0
    };
  });
}
...
const workerNames = ["Juan Dela Cruz", "Elena Reyes", "Roberto Santos"];
...
const styles = StyleSheet.create({
  ...
  timeText: { color: palette.outline, fontSize: 11, lineHeight: 14, fontWeight: "700" },
